...
window.electron.ipcRenderer.on('add-playlist-item', async (_, items: PlaylistItemDto[]) => {
  setNewPlaylistItemsOrderIndexes(items)

  await window.electron.ipcRenderer.invoke('addPlaylistItems', items)

  addNewPlaylistItems(items)

  playlistState.items.filter(x => items.some(y => y.id === x.id)).map(i => loadVideoMetadata(i))
})
...
const getVideoMetadata = async (playlistId: string, playlistItemId: string, videoPath: string): Promise<VideoMetadata> => {
  return await window.electron.ipcRenderer.invoke('getPlaylistItemVideoMetadata', playlistId, playlistItemId, videoPath) as VideoMetadata
}

const playItem = (item: PlaylistItem) => {
  const restartTime = item.progressPercent !== undefined && item.progressPercent >= 100 ? 0 : (item.currentTime ?? 0)
...
const deleteItem = async () => {
  if (currentPlaylistItem.value) {
    await window.electron.ipcRenderer.invoke('deletePlaylistItem', currentPlaylistItem.value.id)
...
const openContainingFolder = (item: PlaylistItem) => {
  window.electron.ipcRenderer.send('open-containing-folder', item.path)
}
...
const updatePlaylistItemOrder = async (event: { moved: { element: PlaylistItem; newIndex: number, oldIndex: number } }) => {
  if (event.moved) {
    ...
    await window.electron.ipcRenderer.invoke('updatePlaylistItem', element.id, { orderIndex: element.orderIndex  })

    if (rebalanceOrder) {
      await window.electron.ipcRenderer.invoke('rebalancePlaylistOrder', playlistState.id)
    }
  }
}
</script>
            ...
            <div class="playlist-item-content">
              ...
              <div class="playlist-item-footer">
                ...
                <div class="playlist-time-info">
                  <span v-if="element.currentTime">
                    {{ formatTime(element.currentTime) }} watched
                  </span>
                </div>
              </div>
            </div>
